...
const VERBOSE = false; // Stopgap, use logger
...
class GooseBay {
    ...
    getOauthClient() {
        ...
        if (VERBOSE) {
            ...
            console.dir({environment: oauthConfig.environment});
        }
    ...
    async refresh(force) {
        ...
        if (this.authResponse) {
            ...
            try {
                ...
                if (VERBOSE) console.dir(authResponse, {depth: Infinity, color: true});
     ...
     */
    setRefreshToken() {
        if (VERBOSE) console.log('The Response is  ' + JSON.stringify(this.authResponse.getJson()));
        if (VERBOSE) console.dir(this.authResponse, {color: true, depth: Infinity});
    ...
    async import(data) {
        ...
        const fetchExisting = await this.qboFindTimeActivities([
            ...
            // TODO: restrict this to only the dates in range for the requested import.
        ]);
        ...
        if (VERBOSE) console.dir(existing, {color: true, depth: Infinity});
        console.log(`Fetched ${Object.entries(existing).length} existing entries.`);
        this.tz = this.config.get('tz') || 'Etc/UTC';
        const p = {};
        console.log(`${data.length} raw time entries to process`);
        for (const n of data) {
            // Log raw data
            ...
            if (VERBOSE) console.dir({ tags, item_id, item_name, customer_id, customer_name, rate, mainTag });
        ...
        // Log the cooked import data
        if (VERBOSE) console.dir(p, { depth: Infinity, color: true });
        ...
        console.log(`${Object.entries(p).length} coalesced candidate time entries`);
        for (const [TxnDate, kinds] of Object.entries(p)) {
            // Log the candidate date
            if (VERBOSE) console.log('=' + TxnDate);
            for (const [k, o] of Object.entries(kinds)) {
                // Log the candidate key.
                if (VERBOSE) console.log(' t: '+k);
                ...
                // Log the candidate request, BEFORE we see if it is applicable
                if (VERBOSE) console.dir(req);
                const skipKey = `c${o.customer_id}:i${o.item_id}`;
                if (!req.HourlyRate || !req.CustomerRef.value || !req.ItemRef.value) {
                    // This is noted at the end. No reason to print twice.
                    if (VERBOSE) console.log('Missing (rate/customer/item) ' + req.TxnDate + ' ' + skipKey );
                    missings.add(req.TxnDate + ' ' + req.Description);
                } else if (existing[req.TxnDate]?.has(skipKey)) {
                    // This is summarized at the end. No reason to note it twice.
                    if (VERBOSE) console.log('Skipping (exists) ' + req.TxnDate + ' ' + skipKey );
                    skips.add(req.TxnDate + ' ' + skipKey);
                } else if (!DRY_RUN) {
                    // Note the candidate we are ABOUT to update.
                    console.dir(req);
                    try {
                        const r = await this.qboCreateTimeActivity(req);
                        // Log the response from teh update request
                        console.dir(r, {color: true, depth: Infinity});
                        upds.add(req.TxnDate);
                    } catch(e) {
                        // Log the error from failing to update.
                        console.error(e);
                        fails.add(req.TxnDate);
                    }
                } else {
                    // Note the candidate we WOULD have updated.
                    console.dir(req);
                    upds.add(req.TxnDate);
                }
            }
